...
class ETinyMce extends CInputWidget
{
	 ...
	 *
	 ...
	 /**
		...
		* @var string
		*/
	 private $contentCSS = ''; # Yii::app()->theme->baseUrl;/css/template.css |	/assets.php?type=css&lang=lv&version=0
	 ...
	 /**
		...
		* @var array
		*/
	 private $validEditorTemplates = array('none', 'simple', 'custom', 'full', 'board', 'basic');
	 ...
	 public function setLanguage($value)
	 {
			...
			if (in_array($lang, $this->validLanguages)) {
				 ...
				 $suffix = empty($lang) ? Yii::app()->language : ($p !== false) ? strtolower(substr($lang, 0, $p)) : strtolower($lang);
	 ...
	 protected function makeNoneEditor()
	 {
			$this->plugins = array('preelementfix', 'autoresize','safari','pagebreak','style','layer','table','save','advhr','inlinepopups','insertdatetime','preview','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
			$options = array();
			
			$options['theme'] = 'advanced';
			$options['skin'] = 'o2k7';
			$options['skin_variant'] = 'black';
			//$options['theme_advanced_toolbar_location'] = 'lef';
			...
			$options['theme_advanced_toolbar_align'] = "'left'";
			$options['theme_advanced_toolbar_location'] = 'bottom';
			$options['theme_advanced_toolbar_align'] = 'left';
	...
	/**
	 * A custom editor, minimal options
	 *
	 * @link http://tinymce.moxiecode.com/examples/simple.php
	 *
	 * @return array
	...
	protected function makeCustomEditor($url='')
	{
			...
			$this->plugins = array('preelementfix', 'paste', 'autoresize','safari','pagebreak','style','layer','table','advhr','advimage','advlink','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
			...
			$options['theme_advanced_toolbar_align'] = 'left';
			$options['theme_advanced_path_location'] = 'bottom';
		$options['theme_advanced_buttons1'] = "print,|,forecolor,backcolor,|,bold,italic,underline,strikethrough,|,sub,sup{$c1},|,pastetext,pasteword,|,removeformat,cleanup,|,visualaid,visualchars,|,ltr,rtl,|,code,preview,fullscreen";
		$options['theme_advanced_buttons2'] = "{$sc}formatselect,fontselect,fontsizeselect,justifyleft,justifycenter,justifyright,justifyfull,|,bullist,numlist,|,outdent,indent";
		$options['theme_advanced_buttons3'] = "undo,redo,|,hr,advhr,nonbreaking,pagebreak,blockquote,|,charmap,emotions,media,image,|,link,unlink,anchor,|,insertdate,inserttime,|,search,replace,|,spellchecker";
	 ...
	 * @return array
	 */
	protected function makeBasicEditor($url='')
	{
			$options = array();
			...
			$options['theme_advanced_toolbar_align'] = 'left';
			$options['theme_advanced_path_location'] = 'bottom';
		$options['theme_advanced_buttons1'] = "forecolor,|,bold,italic,underline,|,pastetext,pasteword,|,removeformat,|,justifyleft,justifycenter,justifyright,justifyfull,|,bullist,numlist,|,outdent,indent,|,code";
		$options['theme_advanced_buttons2'] = "";
		$options['theme_advanced_buttons3'] = "";
		//$options['theme_advanced_buttons4'] = "{$c2}tablecontrols,|,insertlayer,moveforward,movebackward,absolute,|,styleprops,del,ins,attribs,|,template";
		$options['theme_advanced_toolbar_location'] = "'top'";
	 ...
	 * @return array
	 */
	protected function makeBoardEditor($url='')
	{
		$options = array();
		$this->plugins = array('preelementfix', 'paste', 'safari','pagebreak','style','layer','table','advhr','advimage','advlink','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
			...
			$options['theme_advanced_toolbar_align'] = 'left';
			$options['theme_advanced_path_location'] = 'bottom';
		//$options['theme_advanced_buttons1'] = "forecolor,backcolor,|,bold,italic,underline,strikethrough,|,sub,sup{$c1}";
		$options['theme_advanced_buttons1'] = "forecolor,|,bold,italic,underline,fontsizeselect,pastetext,pasteword,|,removeformat,|,justifyleft,justifycenter,justifyright,justifyfull,|,bullist,numlist,|,outdent,indent,|,code";
		//$options['theme_advanced_buttons2'] = "{$sc}formatselect,fontselect,fontsizeselect,justifyleft,justifycenter,justifyright,justifyfull";
		$options['theme_advanced_buttons2'] = '';
		$options['theme_advanced_buttons3'] = '';
		//$options['theme_advanced_buttons4'] = "{$c2}tablecontrols,|,insertlayer,moveforward,movebackward,absolute,|,styleprops,del,ins,attribs,|,template";
		$options['theme_advanced_toolbar_location'] = "'top'";
		...
		$options['theme_advanced_statusbar_location'] = "bottom";
			...
			//$options['force_br_newlines'] = true;
	...
	/**
	 ...
	 protected function makeFullEditor($url='')
	 {
			...
			$this->plugins = array('preelementfix', 'autoresize','safari','pagebreak','style','layer','table','save','advhr','advimage','advlink','emotions','spellchecker','inlinepopups','insertdatetime','preview','media','searchreplace','print','directionality','fullscreen','noneditable','visualchars','nonbreaking','xhtmlxtras','template');
	...
	protected function makeOptions($url='')
	{
			...
			if ($this->editorTemplate !== '') {
				 switch ($this->editorTemplate) {
						case 'none':
							 $template = $this->makeNoneEditor();
							 break;
						case 'simple':
							 $template = $this->makeSimpleEditor();
							 break;
						case 'custom':
							 $template = $this->makeCustomEditor();
							 break;
						case 'board':
							 $template = $this->makeBoardEditor();
							 break;
						case 'full':
							 $template = $this->makeFullEditor($url);
							 break;
						case 'basic':
							 $template = $this->makeBasicEditor($url);
							 break;
				 }
...
EOP;
		...
		if($this->hasModel()) {
			...
			if ($this->useSwitch && !$this->readOnly) {
				 ...
				 $uri = rawurlencode('jQuery("#'.$id.'").toggleModeTinyMCE('.$jsToggleLabels.')');
